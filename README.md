...
- **폴더 ↔ Treemap 연동**
  ...
  - **경로 직접 입력**: 경로 막대의 빈 곳을 누르거나 ✎ / Ctrl+L. 붙여넣기 · `%환경변수` · 대소문자 무시 · `Tab` 폴더 자동완성(스캔 결과에서 즉시). 파일 경로를 넣으면 그 폴더로 가서 파일을 선택한다
    (`ViewModels/PathBarViewModel.cs`, `NodeStore.ResolvePath`)
    - 스캔 이후 생긴 경로는 가까운 상위 폴더를 새로고침해서 열고, 없는 경로는 알리고 가장 가까운 폴더를 연다
    - **스캔 범위 밖**이면 안내와 함께 **[이 경로 스캔]** / **[빠른 이동에서 열기]** 를 제안한다
  - **즐겨찾기**: ☆(지금 폴더 추가·제거, Ctrl+D) · ▾(목록, 우클릭으로 빼기) · 폴더 목록/Treemap 우클릭 메뉴. **빠른 이동 탭과 같은 목록**이다.
    폴더가 지워졌거나 옮겨진 즐겨찾기는 클릭을 기다리지 않고 자동으로 빼고, 빠른 이동 화면이 보일 때 어떤 경로였는지 한 번 알린다(USB·네트워크가 연결되지 않은 것은 지우지 않고 흐리게만 표시)
  - **변경 감지 + 폴더 새로고침** (`Core/Scanning/FolderRefresher.cs`, `MainViewModel.Folder.cs`): 스캔 결과는 스캔 시점의 스냅샷이다.
    폴더를 열 때 · 창으로 돌아올 때 · 열어 둔 폴더가 밖에서 바뀔 때 그 폴더의 **직속 항목만** 실제와 비교해 `⚠ 변경 감지 · 파일 +2 −1 … · F5 새로고침` 으로 알린다(저장소는 건드리지 않는다)
    - **F5** = 이 폴더만 새로고침: 사라진 항목 · 크기 변경 · 새 파일을 반영하고, 새로 생긴 하위 폴더는 안쪽까지 읽는다. 상위 폴더 크기 · Treemap · 큰 파일 · 파일 유형까지 함께 갱신
    - **Shift+F5** = 이 폴더 **하위 전체**를 다시 읽는다(기존 하위 폴더의 안쪽 변경까지). **Ctrl+F5** = 드라이브 전체 재스캔(기존)
    - 비교 기준은 파일 크기다(수정 시각만 다른 것은 세지 않는다 — MFT/열거 스캔 방식에 따른 시각 오차로 항상 "변경"이 뜨는 것을 막는다)
